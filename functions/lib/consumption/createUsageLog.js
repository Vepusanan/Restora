 ...
 */
exports.createUsageLog = (0, https_1.onCall)({
    region: 'us-central1',
    timeoutSeconds: 60,
    memory: '256MiB',
}, async (request) => {
    ...
    const candidateBatchIds = Array.isArray(data?.candidateBatchIds)
        ...
        : [];
    if (!ingredientKey) {
        throw new https_1.HttpsError('invalid-argument', 'ingredientKey is required');
    }
    if (!(0, types_1.isValidUsageCategory)(category)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid usage category');
    }
    if (!Number.isFinite(quantityUsed) || quantityUsed <= 0) {
        throw new https_1.HttpsError('invalid-argument', 'Quantity must be greater than 0');
    }
    ...
    if (user.status !== 'approved') {
        throw new https_1.HttpsError('permission-denied', 'Only approved users can log usage');
    }
    const ids = candidateBatchIds.length > 0
        ? candidateBatchIds
        : batchId
            ? [batchId]
            : (await db
                ...
                .get()).docs.map((docSnap) => docSnap.id);
    if (ids.length === 0) {
        throw new https_1.HttpsError('failed-precondition', 'No batches found for this ingredient');
    }
    const usageGroupRef = db.collection('inventory_usage').doc();
    const usageGroupId = usageGroupRef.id;
    const result = await db.runTransaction(async (tx) => {
        const snaps = await Promise.all(ids.map((id) => tx.get(db.collection('inventoryBatches').doc(id))));
        ...
        for (const snap of snaps) {
            ...
            if (String(batch.ingredientKey ?? '').toLowerCase() !== ingredientKey)
                continue;
            ...
            liveCandidates.push({
                ...
                createdAt: typeof batch.createdAt?.toDate === 'function'
        ...
        let confirmed;
        try {
            confirmed = (0, types_1.allocateFifoConsumption)(liveCandidates, quantityUsed, {
                allowExpired,
                batchId,
            });
        }
        catch (error) {
            throw new https_1.HttpsError('invalid-argument', error instanceof Error ? error.message : 'Unable to allocate usage');
        }
        ...
        for (const alloc of confirmed) {
            ...
            if (alloc.quantityUsed > remaining) {
                throw new https_1.HttpsError('invalid-argument', `Only ${remaining} ${batch.unit ?? ''} remaining on a selected batch`);
            }
            const unitCost = Number(batch.unitCost ?? 0);
            const consumptionCost = (0, types_1.calculateConsumptionCost)(alloc.quantityUsed, unitCost);
            totalCost += consumptionCost;
            const nextQuantity = (0, types_1.roundQty)(remaining - alloc.quantityUsed);
    ...
    firebase_functions_1.logger.info('createUsageLog success', {
...
//# sourceMappingURL=createUsageLog.js.map